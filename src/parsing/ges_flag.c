...
int parsing_arg(key_p *key, char **arg)
{
    ...
    for (int r = 1; arg[r] != NULL; r++) {
        for (i = 0; key_arg[i].name_full != NULL; i++) {
            if (my_strncmp(key_arg[i].name_full, arg[r], my_strlen(key_arg[i].name_full)) == 0) {
                act += key_arg[i].function(key, recup_key(arg[r]));
                break;
            } if (key_arg[i].name_last != NULL && my_strncmp(key_arg[i].name_last, arg[r], my_strlen(key_arg[i].name_last)) == 0) {
                if (key_arg[i].number_arg == 0) {
                    act += key_arg[i].function(key, NULL);
                    break;
                } else if (arg[r + 1] != NULL) {
                    act += key_arg[i].function(key, arg[r + 1]);
                    r += 1;
                    break;
                }
            }
        }
        if (key_arg[i].name_full == NULL) {
